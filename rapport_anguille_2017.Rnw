...
au barrage d'Arzal\\ Rapport 2017 )}, pdfauthor={Cédric Briand},
pdfsubject={anguille},
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_anguille/image/2017/}}
...
au barrage d'Arzal, rapport 2017}
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année 2017
...
# Il faut lui demander et les coller dans data/2017
...
vil<-rbind(cbind("value_std"=NA,"year"=1960:1970),
    vil,
    cbind("value_std"=NA,"year"=c(2012:2014,2016,2017))
)
...
\begin{table}[htpb]
\centering
\caption{Captures de la pêcherie de civelles d'Arzal de 1995 à 2017, sources : 
...
p<-ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
	...
	theme(panel.border = element_blank(),
		axis.line = element_line())+
	scale_x_datetime(limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2018-01-01"))))+
	xlab("Date")
print(p)
dev.off()
...
\begin{figure}[!ht]
  ...
  \caption{Tendance saisonnière des poids moyens de 2006 à 2017, calée avec une
...
\begin{figure}[!ht]
  ...
  \caption{Tendance saisonnière des poids moyens de 2006 à 2017. Ajustement par
...
\begin{figure}[htbp]
  ...
  \caption{Réponses du modèle ajusté manuellement pour les années 2008 à 2017,
...
\begin{figure}[htbp]
  ...
  2017, les points représentent les poids moyens mesurés (en fonction des effectifs), et les courbes les
...
La figure \ref{figure_barchart_bmi_agj} montre une migration 2017 modale centrée sur le
mois de juillet, qui est le seul avec un effectif supérieur à la moyenne historique. 
boxplot_taille_agj.png
...
deux passes d'Arzal (Figure \ref{str_taille_mois_redress2017}). La taille
...
\ref{str_taille_mois_redress2015} et  
\ref{str_taille_mois_redress2016}
) on observe clairement la prédominance des anguilles de l'année en 2013 et
2014, et des arrivées plus diffuses en 2010, 2011, 2012 et 2016. En 2017, le
mode correspondant aux civelles de petite taille est absent.
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
