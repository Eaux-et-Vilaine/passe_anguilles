...
au barrage d'Arzal. Rapport 2021}, pdfauthor={Cédric Briand},
pdfsubject={anguille},
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_anguille/image/2021/}}
...
au barrage d'Arzal, rapport 2021.\\
\vspace{2cm}\\
...
\vspace{2cm}
\includegraphics[width=\paperwidth]{../eel.jpg}
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année 2021
...
\caption{La passe à anguilles du gabion, passe principale 1996-2021.}
...
\caption{La passe à anguilles du mur guide eau, passe secondaire, 2007-2021.}
...
g<-g+annotation_custom(g1_grob, xmin=1985, xmax=2021, ymin=50, ymax=200)
print(g)
#ggthemr_reset()
dev.off()
@
...
vil<-rbind(cbind("value_std"=NA,"year"=1960:1970),
		vil,
		cbind("value_std"=NA,"year"=c(2012:2014,2016,2017,2018,2019,2020,2021))
)
...
co <- vil[vil$year==2015,"value_std"]/4.86
...
r_df_hist <- choice_c(r_df,
		...
		horodatefin="2021-12-31",
		silent=TRUE)
r_df_hist <- connect(r_df_hist)
# attention très très long
png(filename=paste0(imgwdy,"report_df_1996_2021.png"),width=8,height=20, units="in",res=300)
...
r_dc<-choice_c(r_dc,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc_1996_2021.png"),width=8,height=20, units="in",res=300)
...
p <- ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
		...
		theme(panel.border = element_blank(),
				axis.line = element_line())+
		scale_x_datetime(date_labels = "%Y",limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2021-01-01"))),date_breaks="2 years")+
		xlab("Date")+
		ylim(0.1,0.5)
print(p)
dev.off()
...
# write_database(bilPM) #  You have written 9497 rows in the database
...
Le détail mensuel de la migration de 2021 (en effectif) est donné en annexe aux
...
deux passes à anguilles d'Arzal (Figure \ref{str_taille_mois_redress2021}). La
...
estuaire. En 2020 et 2021 on observe une bimodalité bien marquée avec des
anguillettes de petite taille < 100 mm correspondant aux arrivées de l'année et
des anguillettes de 150 mm en 2020 puis 160 mm en 2021 correspondant aux
...
#2021 abondance_reelle <- vvv[["AGJ"]][["N"]]$effectif[-length(vvv[["AGJ"]][["N"]]$effectif)]
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_df_1996_2021.png}
\caption{Fonctionnement de la passe de 1996 à 2021.}
\label{report_df_1996_2021}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_dc_1996_2021.png} 
\caption{Bilan du fonctionnement du dispositif de comptage (piège) de 1996 à 2021.}
\label{report_dc_1996_2021}
\end{figure}
...
\textbf{Résumé}\par
    ...
    caractéristiques biologiques des civelles en 2021.\par
    \vspace{8mm}
  \textbf{Abstract}\par
