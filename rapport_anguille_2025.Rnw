...
\usepackage[french]{babel} % guillemets \og text \fg{}
...
\definecolor{vert_clair_BGM}{RGB}{118, 198, 199}
\definecolor{vert_fonce_BGM}{RGB}{5, 70, 88}
%%============================================================
%% BIBLIOGRAPHY SETUP
%%============================================================
...
au barrage d'Arzal. Rapport 2025}, 
...
\graphicspath{{C:/workspace/passe_anguilles/image/2025/}}
...
\Large \textbf{
    {\titlefont Suivi des migrations d'anguilles (\textit{Anguilla anguilla},
    L.) sur la Vilaine.
    \color{vert_clair_BGM} Rapport 2025.}} \par  
 \vspace{0.5cm}
\centering{\Large{\color{vert_fonce_BGM}{\titlefont
Cédric Briand, Brice Sauvaget, Gérard Eriau \par}}}
 \vspace{0.5cm}
 \color{black}{\large{\titlefont Date d'édition :}}
\qquad \color{vert_clair_BGM}\large{\titlefont Juillet 2026}
\end{minipage}
%
...
%
\begin{minipage}{0.28\textwidth}
\centering{\textbf{Réalisé avec le concours de} : \par} 
\end{minipage}
\hfill{
...
\vspace{2cm}
\includegraphics[width=\paperwidth]{../civelles_bac2.2.JPG}

\vspace{2cm}
\begin{minipage}{0.8\textwidth}
\begin{center}
\hspace{3cm}{\color{white}{\huge \itshape{Juillet 2025}}}
\end{center}
\end{minipage}
\restoregeometry
\clearpage
...
obj <- ls(all=TRUE)
obj <- obj[!obj%in%c("pois","passworddistant","passwordlocal","userdistant","hostdistant","umysql","hostmysql","pwdmysql", "mainpass")]
...
\label{Introduction}
    ...
    Le stock d'anguilles a subi depuis le début du siècle dernier un fort
    déclin qui s'est traduit à partir du début des années 1980 par un déclin du
    recrutement (arrivées de juvéniles). Aujourd'hui, l'espèce est considérée
    par L'IUCN comme étant en danger critique d'extinction, et depuis 1998, l'avis
    ...
    (\href{https://doi.org/10.17895/ices.advice.19772374}{avis CIEM}).
    ...
    dévalaison des anguilles argentées est suivie à l'aide d'un didson (sonar
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année
    2025 sur le barrage d'Arzal.
...
construit par l'Institution d'Aménagement de la Vilaine en 1970, et qui bloque
l'onde de marée à 10 km de l'embouchure de l'estuaire.
L'estuaire en amont, jusqu'à la limite historique de l'estuaire tital 
(barrages de la Potinais et de Malon, respectivement à 45 et 77 km en amont)
a été transformé en rivière. Le plan d'eau ainsi créé a vu se développer de
nouveaux usages dont les principaux sont la navigation et l'eau potable.
Le barrage d'Arzal joue aussi un rôle majeur dans la protection contre les crues de l'aval du bassin versant, 
en empêchant les conjonctions de forts débits amont et de surcotes de marées.
Ce barrage constitue ainsi une rupture nette entre des eaux saumâtres et douces. 
Il évacue le débit de la Vilaine qui avec \num{12400} km$^2$ couvre le tiers de
la surface de la Bretagne.
...
donnent des informations précises sur les captures de la pêcherie.
...
l'exutoire de la passe à fentes verticales, en rive gauche du pertuis des vannes
(Voir Figure \ref{figure_plan_localisation}).
Cette passe dispose de deux rampes. 

La première est accrochée au bajoyer du
barrage, et plonge dans l'estuaire près des vannes (Figure
\ref{figure_passe_gabion} C). Elle était sensée être placée au niveau d'un
contre courant, à un endroit où les navires de pêche rasant le barrage
réalisaient les meilleures prises.
En pratique elle est peu efficace, car les courants générés par les volets (ou clapets de surface)
et les vannes (lorsque les
...
point au nord représente la passe en rive droite, située sur le mur guide eau à
l'entrée de l'écluse.
...
En pratique, ces tuyaux rejettent l'essentiel du débit de la Vilaine lors des
périodes d'étiage, et comme ils sont situés tout près de l'entrée de la rampe aval de la passe à
...
\caption{La passe à anguilles du gabion, passe principale, 1996-2024.}
\label{figure_passe_gabion}
\end{figure}

Pour tenter d'augmenter l'efficacité de la passe à anguilles, une deuxième rampe
a été construite en 2007, de l'autre côté du pertuis de vannes (Figure
...
cette passe capture beaucoup de civelles que celle du gabion, lorsque celles-ci
s'accumulent le long du mur guide eau (Figure \ref{figure_passe_guide_eau} B), 
mais son efficacité est en général plus réduite.
...
\caption{La passe à anguilles du mur guide eau, passe secondaire, 2007-2025.}
\label{figure_passe_guide_eau}
\end{figure}

\subsubsection{Protocole de suivi}

Les opérations de relève sont quotidiennes, week-ends inclus quand les
migrations sont importantes, mais plus espacées lorsque les effectifs d'anguilles jaunes et de civelles sont plus réduits (Figures \ref{bmm},
...
en détail avec stades pigmentaires, taille et poids.
...
de coefficients journaliers de conversion poids-effectif, il est nécessaire de
...
poids moyen. La mesure des poids moyens est faite sur un à trois
sous-échantillons, les civelles sont épuisettées rapidement après leur sortie de l'eau afin de garder
de l'humidité dans l'épuisette. L'objectif lors du suivi est que les civelles ne
perdent pas de mucus lors de leur pesée.

A partir de cette année, le même lot de 50 civelles est pesé deux fois
successivement et le poids moyen est entré en base. Au delà de deux pesées
successives, le poids augmente en raison des civelles qui se fatiguent et
restent plus serrées les unes contre les autres, conservant plus d'eau.
...
\end{aligned}
\end{equation}
Avec $t$ = jour, $T$= 365 jours, $a$, $b$, $c$ paramètres. Le jour $d'$ commence
le premier août.
...
capt <- serie[, c("saison", "capture_pecherie_kg")]
colnames(capt)<-c("Saison","Capture")
...
#ggthemr(fresh2, type="outer", layout="scientific", spacing=0.8)
g <- ggplot(capt)+
   geom_col(aes(x=Saison,y=Capture/1000), fill=bleu_clair_EV)+
   scale_x_discrete(breaks=c("1995_1996","2000_2001","2005_2006","2010_2011","2015_2016","2020_2021")) +
   theme_bw() +
   ylab("Civelle (Tonnes)")
print(g)
dev.off()
...
pêcherie, c'est à dire les montées sur la passe et l'estimation
d'arrivées en estuaire non pêchées et n'ayant pas franchi la passe
...
\caption{Tendances des captures de civelles en estuaire de Vilaine
pour l'année \Sexpr{CY}.
Les échappements estimés à l'aide du modèle GEMAC ou d'une hypothèse
d'efficacité de la passe sont comparés aux captures en estuaire 
(données 1996-2005, \citep{briand_DynamiquePopulationMigration_2009}).}
\label{figure_captures}
\end{figure}

Après 2005, pour estimer l'échappement à la pêche civelière, il serait
...
transposer à la capture des civelles en hiver. Il est nécessaire de rappeler
ici que la mise en place des quotas de pêche a pu se traduire certaines années
par des montées massives de civelles en janvier, alors que de telles montées 
n'avaient jamais lieu quand la pêcherie fonctionnait en continu au pied du
barrage, les nuits de relève et les échappements à la pêcherie ne permettant 
pas aux civelles de disposer du temps nécessaire à leur changement de
comportement les conduisant à utiliser les passes.

\small
\begin{table}[htpb]
\centering
\caption{Captures de la pêcherie de civelles d'Arzal de 1995 à \Sexpr{CY}.
Sources :
...
\end{table}
\normalsize
Les arrivées de civelles sont estimées à l'échelle du stock par
les données d'un ensemble de sites mesurant les arrivées sur le long terme sur l'ensemble du
...
mélange de civelles et de jeunes anguilles jaunes. Ces données sont comparées à la
  ...
  recrutement européen. Les valeurs de 2012 à \Sexpr{CY} pour lesquelles les
  captures ont été influencées par les quotas n'ont pas non plus été inclues (en orange).}
...
La passe du gabion a été relancée le 10 février suite aux travaux de réfection
des gabions aval, puis a fonctionné en continu toute l'année (Figures \ref{bmm_civelle_6}, \ref{report_df_1996_2024}, \ref{report_dc_1996_2024}).

La passe du mur guide eau a également été arrêtée pendant les
travaux des gabions, remise en service le 11 février, puis arrêtée à nouveau
pour les travaux sur l'écluse à partir du 30 octobre. Les autres arrêts consistent en 2
week-ends et 2 semaines pendant les pêches électriques de septembre (Figure \ref{bmm_civelle_12}, \ref{report_dc12_1996_2024}).
...
png(filename=paste0(imgwdy,"report_df_1996_2024.png"),width=8,height=20, units="in",res=300)
...
r_dc<-choice_c(r_dc,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc_1996_2024.png"),width=8,height=20, units="in",res=300)
...
r_dc<-choice_c(r_dc,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc12_1996_2024.png"),width=8,height=15, units="in",res=300)
plot(r_dc1,plot.type="1",main="")
dev.off()
...
#View(bilPM@data[bilPM@data$w>0.45 ,])
...
don$source <-factor(don$source, levels = c("passe", "peche"), labels= c("passe", "estuaire"))
...
p + geom_point(aes(size=effectif))+
		...
		scale_colour_manual(values= c(bleu_EV,turquoise_EV),  breaks = levels(don$source))
dev.off()
...
xtnpm <- xtable(tnpm,
		caption=str_c("Nombre d'échantillons de poids moyen de civelles depuis ",start_year, " et jusqu'au 1er août ",
...
# il y a des données > CY ça fait planter le modèle
bilPM@calcdata[["data"]] <-
   bilPM@calcdata[["data"]][bilPM@calcdata[["data"]]$date < strptime(str_c(CY,"-12-31"), format ="%Y-%m-%d"),]
...
p <- ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
		...
		theme(panel.border = element_blank(),
				axis.line = element_line())+
		scale_x_datetime(date_labels = "%Y",limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2024-02-01"))),date_breaks="2 years")+
		xlab("Date")+
		ylim(0.15,0.55)
print(p)
dev.off()
...
# DECOMMENTER CI DESSOUS POUR ECRIRE!!!!!!!!!!!!!!!
#  ICI ON REECRIT LES COEFFICIENTS EN BASE. Décommenter write database, attention ça prend du temps 
!
 import_coe[import_coe$coe_date_debut==as.Date("2024-08-01"),]
 
# write_database(bilPM) #  You have written 10959 rows in the database
...
load(file = str_c(datawdy,"d2.Rdata"))
#x11()
...
<<test_poids_sec, echo=FALSE, eval=FALSE, results='hide'>>=

#afficher le poids sec par individu pour comparer la tendance avec les poids moyens humides.
d2$loc <-factor(d2$loc, levels = c("Vilaine_ladder", "Vilaine_estuary"), labels= c("passe", "estuaire"))
names(d2)[names(d2) == "loc"] <- "source"

png(file=str_c(imgwdy,"poids_sec.png"),width=14,height=10,unit="cm",res=300)
p <- ggplot(aes(x=date,y=weight_g,colour=source),data=d2) +
		geom_point()+
		xlab("Date")+ylab("Poids sec par individu (g)") +
		theme_bw() +
		scale_colour_manual(values= c(bleu_EV,turquoise_EV),  breaks = levels(d2$source))

print(p)
dev.off()
names(d2)[names(d2) == "source"] <- "loc"
@
...
png(file=str_c(imgwdy,"gamm_check.png"), width = 6, height = 6, units = 'in', res = 300)
gam.check(gamm_s)
dev.off()
...
estuaire (Figure \ref{pm}), lorsque la pêche est continue sans arrêt dans la saison et que le
...
%==========================================================================
 Pour l'année \Sexpr{CY}, on note une forte augmentation du poids humide des
 civelles (Figure \ref{pm}) par rapport aux années précedentes. En comparant
 l'évolution des poids humides à celle des poids secs individuels (Figure
 \ref{ps}), on remarque que la tendance reste la même.
 
 %==========================================================================
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{poids_sec.png}
  \caption{\og Poids secs individuels \fg{} collectés en 
  estuaire de Vilaine sur des échantillons achetés aux pêcheurs de civelles
  (source estuaire) et sur la passe en rive gauche du barrage d'Arzal (source
  passe). Chaque point représente un individu.}
  \label{ps}
  \end{center}
\end{figure}
%==========================================================================
 ...
 depuis un minimum en 2009 jusqu'à un maximum en 2014 (Figure
 \ref{smpm_seasonal2}).
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
  ...
  avec une tendance de long terme et une tendance saisonnière sinusoïde
  (model.type = "seasonal2" dans stacomiR). L'ajustement de long terme est
  correct mais les valeurs extrêmes sont mal ajustées.}
...
%==========================================================================
 ...
 non présenté - basé sur des gam au lieu de sinusoïdes, imposent une tendance
...
L'année 2025 est particulière, avec des migrations
sur la passe du mur guide eau (xx kg) très proches de celles de la passe du
gabion (xxx kg) (Figures \ref{bmm_civelle_6} et \ref{bmm_civelle_12}).
...
débuté entre fin mars et début avril \textcolor{orange_EV}{entre 1996 et
2009} (Figure \ref{figure_seasonal_civelles}). Après l'implémentation des 
plans de gestion, \textcolor{orange_EV}{en 2010 et 2011}, l'exploitation a 
couvert l'ensemble de la saison, car les mesures de gestion saisonnières ont été 
retirées du plan de gestion, dont la nouvelle mesure de régulation était le
quota. Malheureusement, le quota a été basé sur des valeurs de captures qui correspondaient à un taux d'exploitation important
...
vraiment effondrés avec des poids de 5.9 et 2.7 kg (pour des captures de 3 et 3.9 tonnes - Tableau \ref{tableau_captures}). Du point de vue de la saisonnalité, ces deux
années sont donc difficilement interprétables car les effectifs y sont trop
faibles.

\textcolor{orange_EV}{A partir de 2012}, les arrivées de civelles ont connu
une augmentation (Figure \ref{figure_recrutement}).
...
effectifs hebdomadaires en janvier, février ou mars ont pu dépasser 1 million
d'individus (Figure \ref{figure_seasonal_civelles}).
...
\begin{figure}[!ht]
  ...
  Chaque cluster indique l'effectif ayant migré sur une semaine standard, la
  barre horizontale indique les quantiles 5\% et 95\% de la migration.
...
la saison, qui ont permis l'échappement de civelles en janvier et février. Les
civelles destinées au repeuplement ont été pêchées en fin de saison. 
...
Bien que la fermeture soit intervenue le 13 mars, les migrations sont restées
...
En \textcolor{orange_EV}{2020}, la saison de pêche s'est arrêtée le 22 mars, 
mais les montées ne débutent qu'à partir du 9 avril, en quantités modérées.

En \textcolor{orange_EV}{2021}, la saison de pêche s'est arrêtée le 17 mars. On a observé quelques montées 
début mars avant l'arrêt de la pêcherie,
mais les migrations ont débuté réellement à partir du 27 mars. 
Les montées de civelles sont restées bien en deçà des
années précédentes. 

En \textcolor{orange_EV}{2022}, la pêche a été stoppée le 19 mars, après une petite migration de 10 kg 
la première semaine de mars. L'essentiel des montées a été observé fin mars-début avril, et fin avril. 
Les arrivées de civelles ont été très précoces, 
et il y a eu la plus forte migration pour un mois de décembre avec 61 kg lors de la dernière semaine.

En \textcolor{orange_EV}{2023}, les arrivées de civelles ont été
importantes et très précoces par rapport aux années
précédentes avec, fait exceptionnel, une migration pendant tout l'hiver. 
Les mois de décembre 2022 et février constituent des nouveaux records
historiques. La saison de pêche s'est arrêtée le 19 mars, période après laquelle
la migration a été faible. Il est important de noter que le poids moyen des
civelles en 2023 est largement supérieur à ceux relevés depuis le début des
suivis.

Les arrivées semblent avoir été assez précoces en \textcolor{orange_EV}{2024},
et le quota consommation a été atteint fin janvier, dans un contexte de faibles demandes
pour les pêches de repeuplement. Ainsi la fin de la saison professionnelle est
intervenue très tôt par rapport aux autres saisons, dès le 29 février-1er mars.
Malgré cette situation, il n'y a eu sur la passe que peu de
migration lors des interruptions de pêche courant février, 
et peu d’arrivées par la suite en fin de saison, ce qui était inattendu.

\textcolor{orange_EV}{2025}

ouverture saison le 1er novembre, apparemment rien avant décembre, et peu avant
janvier


(Figure
\ref{figure_barchart_bmi_civelles}, \ref{bmm_civelle_6} et \ref{bmm_civelle_12}).


Le détail mensuel de la migration de \textcolor{orange_EV}{2024} (en effectif) est donné en annexe aux
...
\begin{figure}[htbp]
  ...
  tendance historique. En gris = valeurs min, max et moyenne (hist\_mean), en
  bleu = valeurs maximales, en bleu clair = valeurs supérieures à la moyenne, 
  en marron = valeurs inférieures à la moyenne.}
...
\begin{figure}[htbp]
  ...
  les fonctionnements du dispostif de franchissement (DF), 
...
\begin{figure}[htbp]
  ...
  \caption{Bilan migration du piège secondaire, en rive droite. Le bilan montre
  aussi la conversion entre les poids et les effectifs, 
  les fonctionnements du dispostif de franchissement (DF), 
  du piège (DC), et les opérations de contrôle (Op).}
...
colnames(lsm[['12']])  <-  c("Mois","Min*","Moyenne*","Max*","N (2024)")
colnames(lsm[['6']])   <-  c("Mois","Min*","Moyenne*","Max*","N (2024)")
#==================== xtable6 ===================================
xtsm6  <-  xtable(lsm[['6']],caption=str_c("Montées de civelles \\textsf{(en milliers)} dans la passe principale du barrage
						d'Arzal (gabion en rive gauche). Données par mois, \\textsf{N (",CY,")} = effectifs par mois pour ",CY,". 
                        Les trois statistiques * correspondent à la période
						1996-",CY,".
...
#==================== xtable12 ===================================
xtsm12  <-  xtable(lsm[['12']],caption=str_c("Montées de civelles \\textsf{(en milliers)} sur la passe secondaire du barrage 
                        d'Arzal (mur guide eau en rive droite). Données par mois. 
                        Les trois statistiques * correspondent à la période
...
plot(bili, plot.type="seasonal", timesplit="semaine",silent=TRUE) 
dev.off()
...
qui classe cette année comme la \Sexpr{vvv[["AGJ"]][["rank"]]} ème sur \Sexpr{vvv[["AGJ"]][["maxrank"]]}. Les migrations d'anguilles jaunes sont extrêmement variables d'une année sur l'autre. Le minimum des migrations a été connu en 2005 avec seulement 878 anguilles jaunes dans l'ensemble de l'année (Tableau \ref{table_bilanannuel_ang}). Le
...
\end{figure}
%==========================================================================
La majeure partie des effectifs concerne des petites anguilles d'âge 0 et 1,
puis la migration diminue pour les classes suivantes (Figure \ref{civ_ang_cohorte}). 
La saisonnalité des migrations est marquée, avec les périodes les plus
favorables au printemps - début d'été, puis en fin d'été et en automne 
(Figure \ref{figure_saisonnalite_agj}.
Les coefficients de marée (au printemps) et les débits de la Vilaine
(généralement en automne) sont des éléments prédicteurs de l'intensité
migratoire.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\small
...
\begin{figure}[htbp]
  ...
  d'Arzal. Chaque cluster indique l'effectif ayant migré sur une semaine
  standard, la barre horizontale indique les quantiles 5\% et 95\% de la
  migration.
...
La figure \ref{figure_barchart_bmi_agj} montre que la migration \Sexpr{CY}
a été régulière toute l'année, avec des passages supérieurs à la moyenne sur les
quatre premiers mois et en octobre avec l'arrivée de la première crue. Cette
tendance automnale rappelle celle de l'année précédente (2023) où des migrations 
importantes avaient été déclenchées en octobre et novembre par la première crue.

Cette année, la migration hivernale est constituée d'anguilles jaunes de
petite taille < 150 mm. Les anguilles > 150 mm, peu nombreuses, ont migré au
printemps et à l'automne. Les anguillettes d'âge 0 et 1 < 100 mm sont très peu
représentées (Figures \ref{boxplot_taille_agj}, pour la structure par cohorte 
Figure \ref{civ_ang_cohorte}).
Après les fortes arrivées de civelles en 2023, le phénomène observé en 2013, et
à moindre échelle en 2014, avec les importantes migrations d'anguillettes < 100
mm était attendu mais n'a pas eu lieu (Figures \ref{str_taille_mois_redress2013}
et \ref{str_taille_mois_redress2014}). Il est probable que la sédentarisation
estuarienne ait été meilleure en 2012 et 2013 qu'en 2023, et qu'ainsi les
civelles de 2024 ont bénéficié de place disponible en estuaire.
...
\begin{figure}[htbp]
  ...
  tendance historique. En gris = valeurs min, max et moyenne (hist\_mean), 
  en bleu = valeurs maximales, en bleu clair = valeurs supérieures à la moyenne,
  en marron = valeurs inférieures à la moyenne.}
...
xtbilA <- stacomiR::xtable(bilA,
		caption="Migration du stade anguille jaune, sur les
				...
				répartis au pro-rata des effectifs de chaque année. * \\textit{ Les anguilles jaunes migrent principalement sur les rampes à anguilles, mais
				...
				orientent les poissons vers les vitres, et sont difficilement détectables par
...
plot(bilj,plot.type="seasonal",timesplit="semaine", silent = TRUE) 
dev.off()
save(vvv,file=str_c(datawdy,"vvv.Rdata"))
@
\subsection{Structure en taille des migrations d'anguilles jaunes}
...
la passe à bassins mais la quantification des migrations sur les vitres de vidéo
comptage est difficile et incomplète pour les plus petites anguilles (de moins
de 250-300 mm).
En période de migration, des civelles sont parfois observées devant les vitres de la passe, lorsque cette
dernière est fermée à pleine mer. Il est possible que certaines viennent de
l'amont, pour des civelles et des anguillettes relâchées par la conduite d'évacuation de la passe à anguilles. Il semble que l'essentiel correspond à des
civelles rentrées au droit de fuites à l'aval.
Les migrations de petites anguilles dans la passe à bassins, si elles
interviennent parfois, sont quand même limitées par l'importance de la chute aval de la passe à bassins qui est
généralement réglée à 300 mm. Elles n'interviennent en général qu'autour de
...
des petites tailles (Tableau \ref{table_bilanannuel_ang}).
...
Les données de taille individuelle sont réparties par classe de 5 mm 
puis redressées à partir des effectifs mensuels migrant sur les
deux passes à anguilles d'Arzal (Figure
\ref{str_taille_mois_redress2025_in_report}).
Lorsqu'on compare la structure en taille de \Sexpr{CY} aux structures en taille 
des années précédentes (Figures \ref{str_taille_mois_redress2010} à
\ref{str_taille_mois_redress2024}), on observe clairement la prédominance des 
anguilles de l'année en 2013 et 2014, et des arrivées plus diffuses pour les
autres années, à l'exception de 2021, 2023 et 2024 où l'on retrouve des bonnes
proportions d'anguilles d'âge 0, mais en nombre limité. En 2015, 2017 et 2019,
le mode correspondant aux anguilles de petite taille est absent, et il est très 
faible en 2022. En conclusion pour l'année en cours, les migrations d'anguilles
d'âge 0 sont conséquentes, supérieures à la moyenne mais pas exceptionnelles
(Figure \ref{civ_ang_cohorte}.
...
\begin{figure}[htbp]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress2025.png}
  \caption{Structure en taille des anguilles jaunes en \Sexpr{CY} (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress2025_in_report}
  \end{center}
\end{figure}
%==========================================================================
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%==========================================================================
% new version, the file is svg exported by inkscape as pdf_tex
\begin{figure}[htbp]
  \begin{center}
  \def\svgwidth{\columnwidth}
  \includegraphics[width=0.5\textwidth]{civ_ang_cohorte.pdf}
  \caption{Migration des anguilles jaunes et des civelles sur les passes du
  barrage d'Arzal entre 1996 et \Sexpr{CY}. En haut, chaque tuile représente une
  année et une cohorte à l'exception de la cohorte d'âge 0 (en haut) qui est
...
\begin{figure}[htbp]
  ...
  les effectifs de civelles sont portés sur le même graphique que le précédent.}
...
# TRAITEMENT DES STRUCUTRES EN TAILLE (UTILISATION DE LA CLASSE REPORT MIG CHAR)----
...
(p <-  p + geom_tile(aes(fill=classe)) + 			
			xlab("")+ylab("Cohorte")+
			scale_y_continuous(breaks=c(1990,1995,2000,2005,2010,2015,2020),minor_breaks=c(1991:CY))+
			scale_x_continuous(breaks=c(1995,2000,2005,2010,2015,2020),minor_breaks=c(1992:CY))+
			theme_black())
#### CIVELLES
stc=st[1:nbcol,]
...
system2("inkscape",
        args =  c( 
                str_c(imgwdy,"civ_ang_cohorte.svg"),
                 "--export-type=pdf",
                 "--export-latex"))

# inkscape C:/workspace/passe_anguilles/image/2024/CIV_ANG_COHORTE.svg --export-type=pdf --export-latex
...
p <- ggplot(grdata,aes(x=debut_pas,y=effectif_total,fill=DC))+
		...
		scale_fill_manual(values=c("Piege guide eau"="#66A1D2","Piege gabion"="#043C6B","Passe bassins"="#D2A25F"))+ # couleurs à la main...
...
Le package stacomiR fait l'objet d'une maintenance applicative  :
\href{https://forgemia.inra.fr/stacomi}{forge de développement}. Il a été récemment
mis à jour à la version 0.6.1 et une interface web shiny est en cours de
développement.
...
\begin{figure}[!ht]
  ...
  migration est largement dominée par les civelles.}
  \label{bmm}
  \end{center}
\end{figure}
%==========================================================================
...
\begin{figure}[htbp]
  ...
  \caption{Bilan migration des anguilles jaunes dans la passe à bassins du
  barrage d'Arzal. Les migrations vont dans les deux sens, avec des bilans journaliers
...
xtbilA <- stacomiR::xtable(bilA,
		caption="Migration du stade anguille argentée sur les trois
				passes du barrage d'Arzal (Piège RG = passe piège historique
				sur le gabion, Piège RD = passe du mur guide eau). Les effectifs des opérations à cheval sur deux années sont 
				re-répartis au pro-rata des effectifs de chaque année (\\textit{Note : le stade des anguilles est difficile à déterminer
				en suivi vidéo, et la plupart des anguilles migrant dans la passe à bassins sont classées comme jaunes}).",
		label="table_bilanannuel_agg",
...
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress2021.png}
  \caption{Structure en taille des anguilles jaunes en 2021 (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress2021}
  \end{center}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[!ht]
  ...
  \caption{Structure en taille des anguilles jaunes en 2022 (redressée par
  les effectifs mensuels en migration).}
...
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress2023.png}
  \caption{Structure en taille des anguilles jaunes en 2023 (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress2023}
  \end{center}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress2024.png}
  \caption{Structure en taille des anguilles jaunes en 2024 (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress2024}
  \end{center}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress2025.png}
  \caption{Structure en taille des anguilles jaunes en 2025 (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress2025}
  \end{center}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_df_1996_2025.png}
\caption{Fonctionnement de la passe principale du barrage d'Arzal (gabion en
rive gauche) de 1996 à 2025.}
\label{report_df_1996_2025}
\end{figure}
%==========================================================================
%==========================================================================
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_dc_1996_2024.png} 
\caption{Bilan du fonctionnement du dispositif de comptage (piège) de la passe principale du barrage d'Arzal (gabion en
rive gauche) de 1996 à
2024.}
\label{report_dc_1996_2024}
\end{figure}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_dc12_1996_2024.png} 
\caption{Bilan du fonctionnement du dispositif de comptage (piège) de la passe
secondaire du barrage d'Arzal (gabion en rive droite) de 2007 à
2024.}
\label{report_dc12_1996_2024}
\end{figure}
...
\midrule
2023 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ER5GHvMhd7hIm7U0xgXtt5YBzTVgcGsewPAMu5IUjMxGKQ?e=oJfWao}{Gestion
de l'anguille sur le bassin versant de la Vilaine, 2023}\\
2022 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ESZvGDLLuz9KiIVdrwVnVYoBQSDRPKLAm-CiNwjjwmFVjw?e=b6YyVl}{Gestion
de l'anguille sur le bassin versant de la Vilaine, 2022}\\
2021 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EabAMF4YvrJJhxYU8-pm6JwBSLrjt05hvlhTO2kxb84Aag?e=jin4mN}{Gestion
de l'anguille sur le bassin versant de la Vilaine, 2021}\\
2020 & 
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/Efo5G35HM99Dq-0cqLe4DrgBjXye8FjC1yNOflilC_NmFw?e=Hadm11}{Gestion
...
2010 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ESLi1jHYH1RHjpI3cSoMCyUB1y6mOJZ87qmc5EyIzHvAQg?e=5FPiha}{Gestion
...
\textbf{Résumé}\par
    ...
    caractéristiques biologiques des civelles en 2024.\par
    \vspace{8mm}
  \textbf{Abstract}\par
