...
\caption{La passe à anguilles du gabion, passe principale, 1996-2025.}
...
png(filename=paste0(imgwdy,"report_df_1996_",CY,".png"),width=8,height=20, units="in",res=300)
...
r_dc<-choice_c(r_dc,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc_1996_",CY,".png"),width=8,height=20, units="in",res=300)
...
r_dc<-choice_c(r_dc,
		12,
		horodatedebut="1996-01-01",
		horodatefin="2025-12-31",
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc12_1996_2025.png"),width=8,height=15, units="in",res=300)
plot(r_dc1,plot.type="1",main="")
dev.off()
...
p <- ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
		...
		theme(panel.border = element_blank(),
				axis.line = element_line())+
		scale_x_datetime(date_labels = "%Y",limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2025-02-01"))),date_breaks="2 years")+
		xlab("Date")+
		ylim(0.15,0.55)
print(p)
dev.off()
...
!
 import_coe[import_coe$coe_date_debut==as.Date("2025-08-01"),]
...
En \textcolor{orange_EV}{2025}, la saison de pêche est ouverte le 1er novembre,
il n'y a apparamment pas de captures avant décembre, et les captures restent
faibles avant janvier.
...
Le détail mensuel de la migration de \textcolor{orange_EV}{2025} (en effectif) est donné en annexe aux
...
colnames(lsm[['12']])  <-  c("Mois","Min*","Moyenne*","Max*","N (2025)")
colnames(lsm[['6']])   <-  c("Mois","Min*","Moyenne*","Max*","N (2025)")
#==================== xtable6 ===================================
...
\ref{str_taille_mois_redress2025}), on observe clairement la prédominance des 
...
# inkscape C:/workspace/passe_anguilles/image/2025/CIV_ANG_COHORTE.svg --export-type=pdf --export-latex
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_dc_1996_2025.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.5\textwidth]{report_dc12_1996_2025.png} 
...
\textbf{Résumé}\par
    ...
    caractéristiques biologiques des civelles en 2025.\par
    \vspace{8mm}
  \textbf{Abstract}\par
