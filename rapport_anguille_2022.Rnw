...
\end{table}
\normalsize
Les arrivées de civelles \rev{sont estimées} \rev[red]{à l'échelle du stock} par
...
Mais les montées ne débutent en grande quantité que à partir du 10 avril. Les
montées mensuelles de civelles restent bien en deça des années précédentes, sauf
pour le mois d'avril (Figure 15).
