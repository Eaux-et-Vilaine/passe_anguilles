...
height=\paperheight,%
keepaspectratio]{../2022/frontpage.png}%
%\vfill
}}}%
...
Les
opérations de relève sont quotidiennes, week-ends inclus quand les migrations
sont importantes, mais plus espacées lorsque les effectifs d'anguilles jaunes et de civelles sont plus réduits (Figures \ref{bmm},
...
informations sur les caractéristiques biométriques des civelles. Ainsi, on
dispose des poids moyens humides (Figure \ref{pm}) et également des poids secs
individuels (Figure \ref{ps}).
...
En \textcolor{orange_EV}{2022}, aucune migration sur la passe n'a eu lieu
fin janvier ni début février, puis on observe quelques montées avant
l'arrêt de la saison de pêche le 19 mars. Contrairement à la moyenne historique,
l'essentiel de la migration se concentre au mois d'avril (Figure
\ref{figure_barchart_bmi_civelles}). Trois pics successifs de migration sont
observés fin mars et mi-avril avec le pic du milieu moins marqué sur le gabion
qui correspond à un fonctionnement en volets sur le volet 5 (coté Gabion) et un
arrêt des volets du coté du mur Guide eau. Puis la migration s'est
tarrie sur le mur guide eau alors qu'elle s'est poursuivie en mai et juin sur le gabion 
(Figures \ref{bmm_civelle_6} et \ref{bmm_civelle_12}). La migration de décembre
est la plus forte jamais observée (Figure \ref{figure_barchart_bmi_civelles}).
...
\begin{figure}[htbp]
  ...
  aussi la conversion entre les poids et les effectifs, les fonctionnement du dispostif de franchissement (DF), du
  piège (DC), et les opérations de contrôle (Op).}
...
tab[,2] <- round(tab[,2])
tab[,3] <- round(tab[,3],1)
xtbili6 <- xtable(tab,caption="Montées de civelles sur la passe principale du barrage d'Arzal (gabion en rive gauche), $N$ nombre,
				$P$ poids (en kg).",
...
qui classe cette année comme la \Sexpr{vvv[["AGJ"]][["rank"]]} ème sur \Sexpr{vvv[["AGJ"]][["maxrank"]]}. Les migrations d'anguilles jaunes sont extrêmement variables d'une année sur l'autre. Le minimum des migrations a été connu en 2005 avec seulement 878 anguilles jaunes dans l'ensemble de l'année (Tableau \ref{table_bilanannuel_ang}). Le
    ...
    L'année \Sexpr{CY} est particulière car la passe a subi un arrêt pendant
    tout le mois de novembre (du 2 novembre au 14 décembre). La pompe a été
    arrêtée pour nettoyage et vérification d'étanchéité. Des réparations ont été
    nécessaires car il y avait de l'eau dans le circuit d'huile, la pompe a été
    vidangée, nettoyée, repeinte (peinture époxy), l'étanchéité a été refaite,
    le tuyau a été changé, et une pompe neuve a été achetée pour remplacement
    ultérieur et pour avoir à disposition une pompe de secours.
...
caractère saisonnier marqué avec une migration très importante en janvier puis
décembre tout à fait inhabituelle et probablement liée aux arrêts de
la pêcherie de civelle pendant la saison de pêche (Figure
\ref{figure_saisonnalite_agj}).
Les débits de la Vilaine et les coefficients de marée sont des éléments prédicteurs de l'intensité migratoire.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\small
...
\begin{figure}[htbp]
  ...
  horizontale indique les quantiles 5\% et 95\% de la migration.
  En couleurs, période entre la première arrivée d'anguilles jaunes et la
  dernière, avec une couleur relative à l'importance des effectifs en migration.
...
inférieure à la moyenne interannuelle. A l'exception des mois d'octobre
et décembre (pour lequel la migration est exceptionnelle), les migrations
mensuelles sont supérieures à la moyenne interannuelle.
Cette tendance contraste globalement avec celle de l'année précédente (2021 
\footnote{ \href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EabAMF4YvrJJhxYU8-pm6JwBSLrjt05hvlhTO2kxb84Aag?e=jin4mN}{
lien vers rapport 2021, P 16 pour comparer les graphes}}) où des migrations
importantes ont été observées, notament de juillet à novembre où les effectifs
étaient supérieurs à la moyenne.
Les migrations estivales correspondent à des anguilles de grande taille (autour
de 180 mm) alors que les migrations de l'été 2021 correspondaient à des
...
C'est donc la cohorte de 2021 qui forme le gros des effectifs et il n'y a pas eu beaucoup
de civelles en estuaire pour constituer la cohorte de 2022 (Figures
\ref{boxplot_taille_agj}, pour la structure en taille Figure \ref{civ_ang_cohorte}).
...
\ref{str_taille_mois_redress2020} et \ref{str_taille_mois_redress2021}), on
observe clairement la prédominance des anguilles de l'année en 2013 et 2014, et des arrivées 
plus diffuses en 2010, 2011, 2012, 2016 et 2018.En 2017 et 2019, 
le mode correspondant aux anguilles de petite taille est absent, 
et il est très faible en 2022. 
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
...
principale du barrage d'Arzal (gabion en rive gauche) de 1996 à 2022.}
