...
au barrage d'Arzal. Rapport 2020 )}, pdfauthor={Cédric Briand},
pdfsubject={anguille},
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_anguille/image/2020/}}
...
au barrage d'Arzal, rapport 2020}
...
obj <- ls(all=TRUE)
obj <- obj[!obj%in%c("passworddistant","passwordlocal","userdistant","hostdistant","umysql","hostmysql","pwdmysql")]
...
require('stacomiR') # pour l'instant install manuelle
require('ggthemr') # installation manuelle
load_library('tables')
...
@
    ...
    caractéristiques biologiques des civelles en 2020. Les traitements et
    l'écriture du rapport sont fait dynamiquement à l'aide du package
    stacomiR \url{http://stacomir.r-forge.r-project.org/}.
  ...
  \textbf{Abstract}
    ...
    eel in 2020. Data treatment and automated report are built using the
...
\label{Introduction}
    ...
    la quatrième vanne du barrage d'Arzal .
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année 2020
...
\caption{La passe à anguilles du Gabion, passe principale 1996-2020.}
...
\caption{La passe à anguilles du mur Guide Eau, passe secondaire, 2008-2020.}
...
<<captures, eval=FALSE, results=hide, echo=FALSE>>=
...
<<figure_recrutement_vilaine, eval=FALSE, results=hide, echo=FALSE>>=
#######################
## graphique presentation Vilaine
##########################
# Ces deux fichiers sont générés par les traitements wgeel de Cédric
# Il faut lui demander et les coller dans data/2020
...
dat1$year <- as.Date(lubridate::dmy(str_c("01","01",dat1$year)))
...
vil<-rbind(cbind("value_std"=NA,"year"=1960:1970),
    vil,
    cbind("value_std"=NA,"year"=c(2012:2014,2016,2017,2018,2019,2020))
)
...
<<report_df, echo=FALSE, eval=FALSE, results=hide>>=
r_df=new("report_df")
r_df<-choice_c(r_df,
		2,
		horodatedebut="2020-01-01",
		horodatefin="2020-12-31",
		silent=TRUE)
Sys.setenv(TZ='GMT')
r_df<-connect(r_df)
...
g<-ggplot(tpp)+
		...
		scale_fill_manual("type",values=c("1"="#40CA2C","2"="#C8B22D","3"="#AB3B26","4"=marron,"5"="#B8B8B8"),
				labels = gettext("Normal oper.","Operational stop","Stop","Dysfunct.","Unknown",domain="R-stacomiR"))+
		scale_colour_manual("type",values=c("1"="#40CA2C","2"="#C8B22D","3"="#AB3B26","4"=marron,"#B8B8B8"),
				labels = gettext("Normal oper.","Operational stop","Stop","Dysfunct.","Unknown",domain="R-stacomiR"))+		
		ylab("Heure")+
		theme(
				...
				legend.key=element_rect(fill="white",colour=bleufonce),
...
r_df_hist<-choice_c(r_df,
		...
		horodatefin="2010-12-31",
		silent=TRUE)
r_df_hist<-connect(r_df_hist)
# attention très très long
png(filename=paste0(imgwd,"report_df_2003_2010.png"),width=6,height=8, units="in",res=300)
...
# attention très très long
png(filename=paste0(imgwdy,"report_df_2011_2020.png"),width=8,height=12, units="in",res=300)
...
<<report_dc, echo=FALSE, eval=FALSE, results=hide>>=
r_dc=new("report_dc")
r_dc<-choice_c(r_dc,
		5,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc_1998_2010.png"),width=8,height=12, units="in",res=300)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<calcul_coeff_conversion, echo=FALSE, eval=F, results=hide >>=
#'========================================================
#' LANCEMENT
...
p <- ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
	...
	theme(panel.border = element_blank(),
		axis.line = element_line())+
	scale_x_datetime(date_labels = "%Y",limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2020-01-01"))),date_breaks="2 years")+
	xlab("Date")+
	ylim(0.1,0.5)
print(p)
dev.off()
...
effectifs hebdomadaires en janvier, février ou mars pourront dépasser 1 million de
...
En 2015, le quota ne permet pas d'arrêts de la pêcherie pendant la
saison.  La pêche s'étend sur toute la durée de la saison et on retrouve 
...
\citep{briand_suivi_2018b}. Avec la reprise de la pêche ensuite
jusqu'à fin avril, la migration est anecdotique.

En 2018, les captures sont importantes dès le début de la saison. La pêche est quasi continue, 
...
Les arrivées durant la saison 2019 ont été très précoces et les captures fortes
dès décembre.
Il y a peu d'interruptions de pêche, et très peu de migration sur les passes. 
Aucun transfert de quota ne retarde la fin de saison qui intervient tôt, le 9 mars. 
La migration sur les passes s'effectue à 80\% sur 2 semaines, la dernière de
mars et la première d'avril.

En 2020, la saison de pêche s'arrête le 22 mars. On observe quelques montées sur
la  passe en février et début en mars avant l'arrêt de la pêcherie. Mais les
montées ne débutent en grande quantité que à partir du 10 avril. Les montées de
civelles restent bien en deça des années précédentes (Figure 
\ref{figure_barchart_bmi_civelles}). 
Les pics de migrations sont relativement synchrones sur les deux passes à
anguilles (Figures \ref{bmm_civelle_6} et\ref{bmm_civelle_12}).

Les résultats pour les trois dernières années sont surprenants, car les
arrêts de la pêcherie sont intervenus bien plus précocément que les autres
années et sont une mesure de gestion importante, correspondant aux
préconisations de "bonne gestion" de la ressource.
Un examen particulier des montées sur la passe du mur guide eau (rive droite) et
du gabion (en rive gauche) montre une diminution de la proportion de captures sur le gabion (la passe principale). 
Une des causes possible pourrait être une
mauvaise répartition des débits entre la rampe amont et la rampe aval de la
passe du gabion \footnote{La rampe amont adossée au bajoyer est globalement peu
efficace, toutefois pour une configuration de débit similaire, l'année 2020 (rapport à venir)
revient à un  \og pourcentage \fg{} normal du ratio entre la passe et le gabion.
}.
Une analyse plus fine des fonctionnements des vannes du barrage et de leur influence
sur les montées respectives du guide eau et du gabion serait également à
conduire. Il est cependant possible que les arrivées tardives aient été faibles.
Le suivi avec marquage recapture entre 1996 et 2005 a montré la grande
variabilité des arrivées tardives et leur influence sur les montées.

Enfin, on compare les arrivées sur une chronique historique, or le recrutement
en civelles est de l'ordre de quatre à six fois moins important qu'en début de
période. Même si en comparaison aux premières années, la pression de pêche a
beaucoup diminué, les montées se font sur la base d'effectif estuariens très
limités.


Le détail mensuel de la migration de 2020 (en effectif) est donné en annexe aux
...
\begin{figure}[htbp]
  ...
  tendance historique (en gris, valeurs min et max et moyenne (hist\_mean), en bleu valeurs maximales, en bleu clair
...
supérieure à la moyenne interannuelle de janvier à avril et également en
septembre.
Les effectifs des mois de juin à août sont inférieurs à la moyenne historique.
...
deux passes à anguilles d'Arzal (Figure \ref{str_taille_mois_redress2020}). La
...
\ref{str_taille_mois_redress2018} et
\ref{str_taille_mois_redress2019}) on observe clairement la prédominance des
anguilles de l'année en 2013 et 2014, et des arrivées plus diffuses en 2010, 2011, 2012, 2016 et 2018. En 2017
...
estuaire. En 2020 on trouve un mode correspondant aux civelles arrivées dans
l'année (en marron migrations de janvier à juin) et celles arrivées l'année
précédente (en bleu migrations de juillet à décembre) centré autour de 90 mm.
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
...
cat("\\begin{table}[!tbp]",file=vvv[["ANG"]][["path_angtaille"]],sep="\n")
...
cat(iconv("\\caption{Moyenne et écart types des tailles d'anguilles $\tau$ par mois, et nombre
			d'échantillons collectés en \\Sexpr{CY}.}", "UTF-8"),file=vvv[["ANG"]][["path_angtaille"]],sep="\n",append=TRUE)
...
#2020 abondance_reelle <- vvv[["AGJ"]][["N"]]$effectif[-length(vvv[["AGJ"]][["N"]]$effectif)]
