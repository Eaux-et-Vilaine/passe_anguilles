...
au barrage d'Arzal. Rapport 2019 )}, pdfauthor={Cédric Briand},
pdfsubject={anguille},
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_anguille/image/2019/}}
...
au barrage d'Arzal, rapport 2019}
...
obj<-ls(all=TRUE)
obj<-obj[!obj%in%c("passworddistant","passwordlocal","hostpostgres","userdistant")]
...
options(sqldf.RPostgreSQL.user = baseODBC[2], 
	...
	sqldf.RPostgreSQL.host =  "db.mercure.eaux-et-vilaine.bzh", # "w3.eptb-vilaine.fr" "localhost"
...
@
    ...
    caractéristiques biologiques des civelles en 2019. Il constitue la première
  ...
  \textbf{Abstract}
    ...
    eel in 2019. It is the first version of an automated report using the
...
\label{Introduction}
    ...
    réalisés auprès de la pêcherie professionnelle et des arrivées en estuaire non capturées , les tendances du
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année 2019
...
\caption{La passe à anguilles du Gabion, passe principale 1996-2019.}
...
\caption{La passe à anguilles du mur Guide Eau, passe secondaire, 2008-2019.}
...
# Il faut lui demander et les coller dans data/2019
...
\begin{table}[htpb]
\centering
\caption{Captures de la pêcherie de civelles d'Arzal de 1995 à 2019, sources : 
...
host <- decrypt_string(hostpostgres,pois)
...
p<-ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
	...
	theme(panel.border = element_blank(),
		axis.line = element_line())+
	scale_x_datetime(date_labels = "%Y",limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2019-01-01"))),date_breaks="2 years")+
	xlab("Date")+
	ylim(0.1,0.5)
print(p)
dev.off()
...
Les arrivées durant la saison 2019 sont très précoces et les captures fortes dès décembre. 
...
Les résultats pour les deux dernières années sont surprenants, car les
arrêts de la pêcherie sont intervenus très tôt et sont une mesure de gestion
importante, correspondant aux préconisations de "bonne gestion" de la ressource.
Un examen particulier des montées sur la passe du mur guide eau (rive droite) et
du gabion (en rive gauche) montre une diminution de la proportion de captures sur le gabion (la passe principale). Une des causes possible pourrait être une
...
Le détail mensuel de la migration de 2019 (en effectif) est donné en annexe aux
...
ggthemr(fresh2, type="outer", layout="scientific", spacing=0.8)
ggplot(tabdetail)+geom_col(aes(x=bjo_annee,y=N,fill=bjo_dis_identifiant))+xlab(iconv("Année","UTF8"))+
...
\begin{figure}[htbp]
  ...
  sont pas attribuées à un stade particulier. 12 mur guide-eau, 5 passe à
  bassins, 6 Gabion.}
...
La figure \ref{figure_barchart_bmi_agj} montre une migration \Sexpr{CY} modale centrée sur le
mois de mai. Tous les effectifs sont largement inférieurs à la moyenne
historique.
...
xtbilA<-stacomiR::xtable(bilA,
	caption="Migration des stades anguilles jaunes, dans les
		trois passes du barrage d'Arzal (P. bassin = passe à fente verticale\footnote{Les anguilles jaunes migrent principalement dans les rampes à anguilles, mais
...
fréquentant la passe à bassins sont juste présentés pour information.}, Piège RG = passe piège historique
...
de amont, pour des civelles et des anguillettes relâchées par la conduite
...
deux passes à anguilles d'Arzal (Figure \ref{str_taille_mois_redress2019}). La
...
\ref{str_taille_mois_redress2017} et
\ref{str_taille_mois_redress2018}) on observe clairement la prédominance des anguilles de l'année en 2013 et
2014, et des arrivées plus diffuses en 2010, 2011, 2012, 2016 et 2018. En 2017
...
#2019 abondance_reelle<-vvv[["AGJ"]][["N"]]$effectif[-length(vvv[["AGJ"]][["N"]]$effectif)]
...
Le package stacomiR fait l'objet d'une maintenance applicative  : \href{R-forge}{
https://r-forge.r-project.org/R/?group_id=1019}. A partir de la version 4.0 de
R, le package comportant les composants d'interface graphique n'est plus
maintenu. Il va falloir rapidement développer une version fonctionnant à l'aide
de shiny pour remplacer l'interface graphique. Dans l'attente, le package n'est
plus disponible sur CRAN. Dans le travail qui reste à faire :
\begin{itemize}
...
\midrule
2018 & 
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EdJmyXnxtgNJjURh0mYAZy8Bvnc6xNGmffcaAPU5XMSZgA?e=FqPwaq}{Gestion
