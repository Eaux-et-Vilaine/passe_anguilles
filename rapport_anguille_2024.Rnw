...
au barrage d'Arzal. Rapport 2024}, pdfauthor={Cédric Briand},
pdfsubject={anguille},
...
height=\paperheight,%
keepaspectratio]{../2024/frontpage.png}%
%\vfill
}}}%
...
\vspace{5mm}
{\color{jaune_EV}{\Large \itshape{Léa Patau, Cédric Briand, Brice
...
\begin{minipage}{0.8\textwidth}
\begin{center}
\hspace{3cm}{\color{white}{\huge \itshape{Avril 2024}}}
\end{center}
\end{minipage}
\restoregeometry
\clearpage
...
L'estuaire en amont, jusqu'à la limite historique de l'estuaire tital (barrages de la Potinais et de Malon, respectivement à 45 et 77 km en amont)
...
de la Vilaine qui avec \num{12400} km$^2$ couvre le tiers de la surface de la
Bretagne.
...
cette passe capture beaucoup de civelles, lorsque celles-ci s'accumulent le
...
Les
opérations de relève sont quotidiennes, week-ends inclus quand les migrations sont importantes, mais plus espacées lorsque les effectifs d'anguilles jaunes et de
...
r_dc<-choice_c(r_dc,
		...
		silent=TRUE)
r_dc1 <- connect(r_dc)
png(filename=paste0(imgwdy,"report_dc_1996_2023.png"),width=8,height=20, units="in",res=300)
...
\begin{figure}[!ht]
  ...
  \caption{\og Poids secs pour chaque individu \fg{} collectés en 
  estuaire de Vilaine sur des échantillons achetés aux pêcheurs de civelles
  (source estuaire) et sur la passe en rive gauche du barrage d'Arzal (source
...
Les arrivées de civelles pour l'année \textcolor{orange_EV}{2024} ont été
importantes et très précoces par rapport aux années
précédentes. Ces montées correspondent à un arrêt de la pêcherie dès fin
décembre 2022 et à une fenêtre de conditions environnementales favorables à la
migration, avec fait exceptionnel, une migration des civelles pendant tout
l'hiver. L'essentiel de la migration a eu lieu
au mois de février qui dépasse largement la moyenne historique (Figure \ref{figure_barchart_bmi_civelles}).
Au niveau des deux passes, quatres pics de migration sont observés début
janvier, début et fin février, et mi-mars (Figures \ref{bmm_civelle_6} et \ref{bmm_civelle_12}). La saison de
pêche s'est arrêtée le 19 mars, période après laquelle la migration a été
faible.
Egalement, il est important de noter que le poids moyen des civelles en 2024
est largement supérieur à ceux relevés les années pré‐
cédentes.
...
caractère saisonnier marqué avec une migration très importante en novembre et supérieure à la moyenne en janvier
(Figures \ref{figure_saisonnalite_agj} et \ref{figure_seasonal_agj}).
Les débits de la Vilaine et les coefficients de
marée sont des éléments prédicteurs de l'intensité migratoire.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\small
...
globalement inférieure à la moyenne interannuelle de mars à juillet et pour les
mois d'août et de décembre.
Le mois de novembre montre une migration exceptionnelle, beaucoup plus
importante que les années précédentes (Figure \ref{figure_seasonal_agj}).

Cette tendance contraste globalement avec celle de l'année précédente (2022)
où des migrations
inférieures à la moyenne ont étés observées (excepté au mois de décembre).
Cette année la migration printanière est consistuée d'anguilles jaunes de petite
taille < 150 mm (Figures \ref{boxplot_taille_agj}, pour la structure en taille Figure
\ref{civ_ang_cohorte}).
(\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ESZvGDLLuz9KiIVdrwVnVYoBQSDRPKLAm-CiNwjjwmFVjw?e=b6YyVl}{Rapport
2022}, et figure \ref{str_taille_mois_redress2021}).
C'est donc la cohorte de 2021 qui forme le gros des effectifs et il n'y
a pas eu beaucoup de civelles en estuaire pour constituer la cohorte de 2022.
...
deux passes à anguilles d'Arzal (Figure \ref{str_taille_mois_redress2024}). La
...
en taille des années précédentes (Figures \ref{str_taille_mois_redress2010} à \ref{str_taille_mois_redress2022}), on
observe clairement la prédominance des anguilles de l'année en 2013 et 2014, et des arrivées 
plus diffuses en 2010, 2011, 2012, 2016, 2018, 2020 et 2021.


En 2015, 2017 et 2019, le mode correspondant aux anguilles de petite taille est
absent, et il est très faible en 2022. En 2024, le
 mode est bien marqué mais on observe tout de même une prédominance des
 anguilles de taille 150 à 200 mm. 
%==========================================================================
\begin{figure}[htbp]
  \begin{center}
...
2020 & 
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EW-2QQTSoD9FpPhbYVu-a3AB9npKbMDot_uzgq5rmyPypQ?e=4mDh9c}{Gestion
