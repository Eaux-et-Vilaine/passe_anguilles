...
g<-g+annotation_custom(g1_grob, xmin=1985, xmax=2024, ymin=50, ymax=200)
print(g)
#ggthemr_reset()
dev.off()
@
...
Points importants
arrivées assez précoces, et le quota consommation a été atteint fin janvier
faibles demandes pour les pêches de repeuplement
fin de la saison professionnelle dès le 29 février-1er mars
peu de migration lors des interruptions de pêche courant février, et peu
d’arrivées par la suite
? pic de migration a eu lieu le 7 avril avec 65 kg


(Figure \ref{figure_barchart_bmi_civelles})
(Figures \ref{bmm_civelle_6} et \ref{bmm_civelle_12}). 
