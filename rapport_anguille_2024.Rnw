...
où elle a été arrêtée pour cause de travaux sur les gabions aval (Figure
\ref{fbmm_civelle_6}).
...
gabions aval (Figure \ref{fbmm_civelle_12}).
...
deux passes à anguilles d'Arzal (Figure \ref{str_taille_mois_redress2024}).
...
\ref{str_taille_mois_redress2023}), on observe clairement la prédominance des 
anguilles de l'année en 2013 et 2014, et des arrivées plus diffuses pour les
autres années, à l'exception de 2021 et de 2024 où l'on retrouve des bonnes
...
faible en 2022.
str_taille_mois_redress2020 EST FAUSSE !
...
<<bilan_migration, echo=FALSE, eval=TRUE, include=FALSE >>=
...
\end{figure}
%==========================================================================
<<bilan_annuels_anguilleargentee, echo=FALSE, eval=TRUE, include=FALSE >>=
require(stacomiR)
# launching stacomi without selecting the scheme or interface
# the following script will load the Arzal dataset if connected to iav schema
