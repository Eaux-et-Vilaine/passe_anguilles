...
pdftitle= {Suivi de l'anguille jaune (\textit{Anguilla anguilla}, L.) en pêche
  électrique sur le bassin de la Vilaine de 1998 à 2016 : un effet à court terme
  des mesures de gestion sur l'aval ?)}, 
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_anguille/image/2016/}}
...
\begin{document}
	...
	\title{Gestion de l'anguille (\textit{Anguilla anguilla}, L.) sur le bassin de la Vilaine}
...
@
    ...
    caractéristiques biologiques des civelles en 2016. Il constitue la première
  ...
  \textbf{Abstract}
    ...
    ladder, which classifies this year as the \Sexpr{vvv[["AGJ"]][["rank"]]} th on
    ...
    eel in 2016. It is the first version of an automated report using the
...
Les montées de civelles et d'anguilles jaunes sont suivies depuis 1996 au
    niveau de la passe à anguille du barrage d'Arzal. Cette série de données,
    ...
    (recruitement estuarien). Les opérations de transport vers le bassin
    ...
    L'objectif de ce rapport est de présenter le bilan du suivi de l'année 2016
...
La passe à anguille du gabion est située au centre de l'ouvrage, près de
...
probablement le plus efficace. Lors des pics de migration, on observe une
...
déterminer l'efficicité respective de chacune des rampes.
...
\end{figure}
La deuxième rampe à anguille de la passe
...
comme ils sont situés tout prêt de l'entrée de la rampe aval de la passe à
anguille du gabion, ils consituent un excellent débit d'attrait, qui vient
...
\caption{La passe à anguille du Gabion, passe principale 1996-2016.}
\label{figure_passe_gabion}
\end{figure}

Pour tenter d'augmenter l'efficacité de la passe à anguille, une deuxième rampe
à été construite en 2008, de l'autre coté du pertuis de vannes, en 2007 (Figure
\ref{figure_passe_guide_eau} B). Cette rampe, au dimensions plus modestes, 
...
\caption{La passe à anguille du mur Guide Eau, passe secondaire, 2008-2016.}
...
échantillon composé au minimum de 600 anguillettes par mois. Une séparation
à l'oeil des anguilles de plus de 16 cm et moins de 16 cm est effectuée afin de
...
Comme il n'est pas possbile de dénombrer les civelles lorsque les migrations
...
perdent pas de muccus lors de leur pesée.

Le même lot de 50 civelles est pesé trois fois successivement et le poids moyen
est entré en base (avec en remarque les trois poids pesés). 
...
##########################
load("C:/workspace/pdata/traitement_stacomi/rapport_anguille/data/2016/dat_ge.Rdat")
load("C:/workspace/wgeeldata/recruitment/2016/data/wger.Rdata")
dat1<-dat
...
vil<-rbind(cbind("value_std"=NA,"year"=1960:1970),vil,cbind("value_std"=NA,"year"=c(2012:2014,2016)))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<calcul_coeff_conversion, echo=FALSE, eval=FALSE,results=hide >>=
#'========================================================
#' LANCEMENT
...
# pointe vers le schéma IAV, par défaut ça doit marcher mais je remets ici
# pour assurer le coup si le rapport est lancé sur un autre ordi
...
p<-ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
	...
	theme(panel.border = element_blank(),
		axis.line = element_line())+
	scale_x_datetime(limits=as.POSIXct(c(as.Date("1996-01-01"),as.Date("2006-01-01"))))+
	xlab("Date")
print(p)
dev.off()
...
p<-ggplot(don)+ geom_point(aes(x=date,y=w,size=effectif),col="#043C6B")+
	...
	theme(panel.border = element_blank(),
		axis.line = element_line())+
	scale_x_datetime(limits=as.POSIXct(c(as.Date("2006-01-01"),as.Date("2017-01-01"))))+
	xlab("Date")
print(p)
dev.off()
...
Au total, en \Sexpr{CY},  \Sexpr{vvv[["PM"]][["CY"]][["N"]]}
échantillons de poids moyen ont été collectés  (Tableau \ref{tnpm}). 
%==========================================================================
...
\begin{figure}[!ht]
  ...
  \caption{"Poids moyens "humides" collectés en 
...
\begin{figure}[!ht]
  ...
  \caption{Tendance saisonnière des poids moyens de 2006 à 2016, calée avec une
  tendance de long terme et une tendance saisonnière sinusoïde (model.type="seasonal2" dans
...
\begin{figure}[!ht]
  ...
  \caption{Tendance saisonnière des poids moyens de 2006 à 2016. Ajustement par
...
\begin{figure}[htbp]
  ...
  \caption{Réponses du modèle ajusté manuellement pour les années 2008 à 2016,
...
\begin{figure}[htbp]
  ...
  2016, les points représentent les poids moyens mesurés (en fonction des effectifs), et les courbes les
...
clémentes, autorisent la migration de civelles sur passe, dont les effectifs
hébdomadaires en janvier, février ou mars pourront dépasser 1 million de
...
L'année 2016 s'inscrit dans un contexte où de nouveau la pêcherie, gérée avec
des arrêts de pêche, laisse échapper des civelles. Les civelles destinées au
repeuplement sont pêchées en fin de saison. Ainsi, les migrations de janvier et
février sont les plus fortes jamais enregistrées pour ces mois (Figure
\ref{figure_barchart_bmi_civelles} - en janvier et février, la couleur bleue
indique que la migration correspond au maximum historique enregistré).
Les migrations de mars, supérieures à la moyenne historique, sont bien en deça
du maximum enregistré en 2014. Les migrations d'avril, mai et juin sont bien
inférieures à la moyenne historique.



Le détail mensuel de la migration de 2016 (en effectif) est donné en annexe aux
...
\begin{figure}[htbp]
  ...
  \caption{Comparaison des migrations de civelles de l'année 2016 à la tendance
...
<<bilan_interannuels_civelle, echo=FALSE, eval=FALSE,results=hide >>=
...
<<bilan_annuels_anguillejaune, echo=FALSE, eval=FALSE,results=hide >>=
require(stacomiR)
# launching stacomi without selecting the scheme or interface
...
anguilles jaunes mesurées sur la passe vont de la taille civelle (minimum 6 cm)
...
comptage est difficile pour les plus petites anguilles. En période de migration
des civelles sont parfois observées devant les vitres de la passe, lorsque cette
...
fuites à l'aval. Les deux sont connus pour intervenir. Les migrations de petites
...
deux passes d'Arzal (Figure \ref{str_taille_mois_redress}). La taille médiane
des anguilles jaunes varie entre 124 et 180 mm.
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{str_taille_mois_redress.png}
  \caption{Structure en taille des anguilles jaunes en \Sexpr{CY} (redressée par
  les effectifs mensuels en migration).}
  \label{str_taille_mois_redress}
  \end{center}
\end{figure}
%==========================================================================
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[!ht]
  ...
  correspodnant aux anguillettes d'âge zéro à droite. En ligne on peut lire
...
r_mig_char<-choice_c(r_mig_char,
	...
	horodatedebut="2016-01-01",
	horodatefin="2016-12-31",
    echantillon="avec",
	silent=FALSE)
...
donnees_taille=r_mig_char@calcdata
...
ndt<-chnames(object=donnees_taille[donnees_taille$annee=="2016",],
...
sum(ndt$lot_effectif>1) # 65 données de taille avec eff>1 en 2016
ndt<-ndt[!is.na(ndt$taille),]
sum(ndt$lot_effectif>1) #0
ndt<-ndt[ndt$car_val_identifiant!="51"|is.na(ndt$car_val_identifiant),] # 0 ceci vire effectivement les "anguilles" mesurées à part
save(ndt, file=str_c(datawd,"ndt.Rdata"))
# load(file=str_c(datawd,"ndt.Rdata"))
...
# Effectif des angll/ang mesurées par mois en 2015
...
for (i in 1:dim(strtail_annee_mois)[1]){
  for (j in 1:dim(strtail_annee_mois)[2]){ #2010
...
# On la remplit de NA
# On la colle à l'autre sur le coté avec un abind
# On la remplit !
# 
...
don<-don[,12:1]
#==============================================================
png(str_c(imgwdy,"str_taille_mois_redress.png"))
...
plot(bMM_Arzal,plot.type="standard",
	color=color,
	color_ope=color_ope,
	main=iconv("Effectif civelles, piège Gabion (rive gauche) en 2016","UTF8"),
	cex.main=1.1,		
	type="h",
	silent=TRUE)
dev.off()
...
plot(bMM_Arzal,plot.type="standard",
	color=color,
	color_ope=color_ope,
	main=iconv("Effectif civelles, mur guide eau (rive droite) en 2016","UTF8"),
	cex.main=1.1,		
	type="h",
	silent=TRUE)
dev.off()
...
plot(bMM_Arzal,plot.type="standard",
	color=color,
	color_ope=color_ope,
	main="Effectif d'anguilles jaunes Gabion (rive gauche) en 2016",
	cex.main=1.1,
	lty=2,
	type="b",		
	silent=TRUE)
...
plot(bMM_Arzal,plot.type="standard",
	color=color,
	color_ope=color_ope,
	main="Effectif d'anguilles jaunes du mur guide eau (rive droite) en 2016",
	cex.main=1.1,
	lty=2,
	type="b",		
	silent=TRUE)
...
plot(bMM_Arzal,plot.type="standard",
	color=color,
	color_ope=color_ope,
	main="Effectif d'anguilles jaunes dans la passe à bassins en 2016",
	cex.main=1.1,
	lty=2,
	type="b",		
	silent=TRUE)
...
ex(vvv$CIV$data)
require(XLConnect)
wb1<-loadWorkbook(str_c("C:/Users/cedric.briand/Desktop/serie_pro_vilaine.xls"))
...
être considérée comme des argentées (Figure \ref{bmm_agj_5}). En 2016 la
migration en septembre, avant l'ouverture des vannes, d'anguilles argentées par
la basse à bassins et très faible. Globalement les migrations d'argentées sont
très minoritaires lorsqu'on les compare aux autres stades de développement
(Figure \ref{bmm}).
...
stacomiR.
\item Traitement des données morphologiques individuelles des civelles. Le
traitement a été effectué dans le cadre d'un poster pour la conférence
anguilles de Londres 2017, ramener ces traitements dans ce rapport.
\item Traitement des données de structures en taille à l'aide de décompositions
polymodales.
\end{itemize}
\clearpage
\onecolumn
