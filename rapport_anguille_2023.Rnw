...
\begin{minipage}{0.8\textwidth}
\begin{center}
\hspace{3cm}{\color{white}{\huge \itshape{Décembre 2023}}}
\end{center}
\end{minipage}
\restoregeometry
\clearpage
...
vraiment effondrés avec des poids de 5.9 et 2.7 kg (pour des captures de 3
tonnes et 3.9 tonnes - Tableau \ref{tableau_captures}). Du point de vue de la saisonnalité, ces deux
...
mais les montées ne débutent en grande quantité que à partir du 10 avril.

En \textcolor{orange_EV}{2021}, la saison de pêche s'est arrêté le 17 mars. On
a observé quelques montées sur la  passe en février et début mars
avant l'arrêt de la pêcherie, mais les montées n'ont débuté en grande quantité
que à partir du 10 avril. Les montées de civelles sont restées bien en deçà des
années précédentes. 

En \textcolor{orange_EV}{2022}, l'essentiel des montées a été observé en trois
pics entre fin mars et début avril.

Les arrivées de civelles pour l'année \textcolor{orange_EV}{2023} ont été
importantes et très précoces (dès début janvier) par rapport aux années
précédentes. Ces montées correspondent à un arrêt de la pêcherie dès fin
décembre et à une fenêtre de conditions environnementales favorables à la
...
Au niveau des deux passes, quatres pics de migration sont observés en début
janvier, début et fin février, et mi-mars (Figures \ref{bmm_civelle_6} et \ref{bmm_civelle_12}). La saison de
pêche s'est arrêtée le 19 mars, periode après laquelle la migration a été faible.
...
Le détail mensuel de la migration de 2023 (en effectif) est donné en annexe aux
...
for (Y in 1996:CY){  
	...
	if (nrow(bM@data)>0 ){
		write_database(bM,silent=FALSE)
	}
}
...
caractère saisonnier marqué avec une migration très importante en novembre et janvier (Figures \ref{figure_saisonnalite_agj} et
...
mois de septembre et decembre.
Le mois de novembre montre un migration assez exeptionnelle,
beaucoup plus importante que les années précedentes (Figure
\ref{figure_seasonal_agj}).
...
\ref{civ_ang_cohorte}).
\rev[red]{Il faut ajouter le lien vers le rapport 2022 sur le sharepoint puis
pointer vers ce lien pour l'instant on pointe sur 2021
(\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EabAMF4YvrJJhxYU8-pm6JwBSLrjt05hvlhTO2kxb84Aag?e=jin4mN}{Rapport
2021}}, et figure \ref{str_taille_mois_redress2021}).
\rev[red]{C'est donc la cohorte de 2021 qui forme le gros des effectifs et il n'y
...
\ref{str_taille_mois_redress2019},
\ref{str_taille_mois_redress2020},
\ref{str_taille_mois_redress2021} et \ref{str_taille_mois_redress2022}), on
observe clairement la prédominance des anguilles de l'année en 2013 et 2014, et des arrivées 
plus diffuses en 2010, 2011, 2012, 2016, 2018, 2020 et 2021. En
2015, 2017 et 2019, le mode correspondant aux anguilles de petite taille est absent,
et il est très faible en 2022. En 2023, le
 mode est bien marqué mais on observe tout de même un prédominance des anguilles de taille 150.
%==========================================================================
\begin{figure}[!ht]
  \begin{center}
...
\begin{figure}[!ht]
  ...
  le nombre de civelles est porté sur le même graphique que le précédent.}
...
\normalsize
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<test, echo=FALSE, eval=FALSE, include=FALSE >>=
bMM <- new("report_mig")
bMM <- choice_c(bMM,
    dc=c(6),
    taxa=c("Anguilla anguilla"),
    stage=c("CIV"),
    datedebut=str_c(2016,"-01-01"),
    datefin =str_c(2016,"-12-31"))
bMM <- charge(bMM)
bMM <- connect(bMM)
bMM <- calcule(bMM)
